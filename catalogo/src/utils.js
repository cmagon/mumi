...
// Convierte una URL de video (YouTube / Vimeo / embed directo) en una URL para <iframe>
export function videoEmbed(url) {
  ...
  // Si ya es un enlace de inserción (contiene /embed o es un iframe src), se usa tal cual
  if (/\/embed|player\.|\/video\//.test(u)) return u
  return u
}
...
export async function confirmarPedidoWA(items, nota, cfg, mayorista = false, intro = '') {
  const total = items.reduce((s, i) => s + precioItem(i, mayorista) * i.cantidad, 0)
  // Registra el pedido iniciado (best-effort)
  try {
    await supabase.from('pedidos_catalogo').insert({
      ...
      total, nota: nota?.trim() || null,
    })
  } catch { /* noop */ }
  const numero = (cfg?.whatsapp || '+573157702180').replace(/[^0-9]/g, '')
  const texto = encodeURIComponent(construirMensajeWA(items, nota, cfg, mayorista, intro))
